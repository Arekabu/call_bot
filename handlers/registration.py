...
async def cmd_register(message: Message, state: FSMContext):
    ...
    await registration_start_service(message=message, state=state)


@router.message(RegistrationStates.waiting_for_email)
async def process_email(message: Message, state: FSMContext):
    """Обработка введенного email"""
    await registration_email_service(message=message, state=state)


@router.message(RegistrationStates.waiting_for_code)
async def process_code(message: Message, state: FSMContext):
    """Обработка введенного кода"""
    await registration_code_service(message=message, state=state)
